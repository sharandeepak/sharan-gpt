...
lib/
  profile.ts               Profile retrieval context for assistant
  firebase-admin.ts        Server-only Firestore client
  rate-limit.ts            Free-text chat rate limiting
  sanitize.ts              Input guardrails
  session-client.ts        Browser session helpers
  session-server.ts        Admin session helpers
...
The visible resume is powered by `data/profile.json`. Updating this file changes the resume panel content without editing React components.
...
Chennai, India  
Email: `sharandeepak32@gmail.com`
